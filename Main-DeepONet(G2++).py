    ...
    for date in tqdm(df_yields.index, desc="Generating data from yield curves"):
        ...
        for _ in range(samples_per_day):
            ...
            if np.isfinite(option_price) and 1e-7 < option_price < 1.0:
                ...
                trunk_data.append(torch.tensor([S, T, K, tau, moneyness, a, b, sigma, eta, rho], device=device, dtype=torch.float32))
...
for epoch in tqdm(range(num_epochs), desc="Training DeepONet(G2++)"):
    model.train()
...
print("\n" + "="*50)
print(f"FINAL DeepONet RESULTS (G2++)")
print("="*50)
